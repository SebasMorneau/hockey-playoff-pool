...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    logger.section('ROUND 2 PREDICTIONS MIGRATION');
    ...
    try {
      ...
      logger.subsection('MAXIME\'S ROUND 2 PREDICTIONS');
      const maximeId = await getUserIdByEmail('max.lepine@outlook.com');
      
      // Maxime: Toronto in 6 matches in round 2
      // First we need to find the series between Toronto and another team in round 2
      await createOrUpdatePrediction(
        maximeId, 
        await getSeriesIdByTeamsInRound('TOR', 'FLA', 2), 
        'TOR', 
        6
      );
      
      logger.success('Maxime\'s round 2 predictions added successfully.');
      
      // Jonathan's predictions for round 2
      logger.subsection('JONATHAN\'S ROUND 2 PREDICTIONS');
      const jonathanId = await getUserIdByEmail('johabsjo@hotmail.com');
      
      // Jonathan: Dallas in 5 vs WN (assuming WPG - Winnipeg Jets?)
      await createOrUpdatePrediction(
        jonathanId,
        await getSeriesIdByTeamsInRound('DAL', 'WPG', 2),
        'DAL',
        5
      );
      
      // Jonathan: Florida in 6 vs TOR
      await createOrUpdatePrediction(
        jonathanId,
        await getSeriesIdByTeamsInRound('FLA', 'TOR', 2),
        'FLA',
        6
      );
      
      // Jonathan: Washington in 6 vs CAR
      await createOrUpdatePrediction(
        jonathanId,
        await getSeriesIdByTeamsInRound('WSH', 'CAR', 2),
        'WSH',
        6
      );
      
      // Jonathan: Vegas in 6 vs EDM
      await createOrUpdatePrediction(
        jonathanId,
      ...
      logger.success('Jonathan\'s round 2 predictions added successfully.');
  ...
  async down(queryInterface, Sequelize) {
    console.log('\x1b[33m%s\x1b[0m', '⚠️ No downgrade actions needed for round 2 predictions.');
  }
}; 